...
function Access({ setAccess }) {
	...
	}, [parientAccess, appointmentAccess, invoicesAccess, paymentsAccess, setAccess])

	return (
		<div className="w-full">
			<h1 className="mb-3 text-black text-sm">Access</h1>
						...
						{datas.map((item) => (
							<tr key={item.id} className="transitions border-border border-b hover:bg-greyed">
								...
								<td className={tdclass}>
									<Checkbox name="read" checked={item.access.read} onChange={item.onChange} />
								</td>
								<td className={tdclass}>
									<Checkbox name="edit" checked={item.access.edit} onChange={item.onChange} />
								</td>
								<td className={tdclass}>
									<Checkbox name="create" checked={item.access.create} onChange={item.onChange} />
								</td>
								<td className={tdclass}>
									<Checkbox name="delete" checked={item.access.delete} onChange={item.onChange} />
								</td>
							</tr>
						))}
					</tbody>
